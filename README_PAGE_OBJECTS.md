...
4) Винести спільну логіку в `base`.
...
- Якщо використовуєте паралельність, краще, щоб `base` створював окремий інстанс драйвера на клас або на метод (за потреби можна перейти на `@BeforeMethod`).
...
- Є `base` із ініціалізацією та завершенням WebDriver, а також налаштованими `implicit` та/або `explicit` wait-ами.
...
- Не дублюйте код: спільне — у `base` або утилітарних класах.
